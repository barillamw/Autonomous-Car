...
void Init_Ports(int use){      //Calls the rest of the initalization functions
  Init_Port1();
  Init_Port2();
  Init_Port3(p3use);
  Init_Port4();
...
void Init_Port2(){ 
  ...
  //    P2IFG &= ~SW2;                  //Clear all P2.6 inerrupt flags
...
void Init_Port3(int use){             
  ...
  case USE_SMCLK:
    ...
    P3SEL1 &= ~SMCLK_OUT; 
    P3DIR  |= SMCLK_OUT;          //Sets Direction 1
    break;
  default: //USE_GPIO
    P3SEL0 &= ~SMCLK_OUT;         //GPIO Operation (00)
    P3SEL1 &= ~SMCLK_OUT;
    P3DIR &= ~SMCLK_OUT;          //Sets Direction to Input
    break;
  }
...
void Init_Port4(){             
  ...
  //    P4IFG &= ~SW1;                  //Clear all P2.6 inerrupt flags
